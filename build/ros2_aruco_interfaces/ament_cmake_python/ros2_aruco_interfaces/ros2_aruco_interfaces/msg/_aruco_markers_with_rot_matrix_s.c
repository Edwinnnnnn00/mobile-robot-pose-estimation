...
bool ros2_aruco_interfaces__msg__aruco_markers_with_rot_matrix__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("ros2_aruco_interfaces.msg._aruco_markers_with_rot_matrix.ArucoMarkersWithRotMatrix", full_classname_dest, 82) == 0);
  }
  ros2_aruco_interfaces__msg__ArucoMarkersWithRotMatrix * ros_message = _ros_message;
  {  // header
  ...
  {  // rot_mat
    PyObject * field = PyObject_GetAttrString(_pymsg, "rot_mat");
    if (!field) {
      return false;
    }
    if (PyObject_CheckBuffer(field)) {
      ...
      double * dest = ros_message->rot_mat.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
      ...
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'rot_mat'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of ArucoMarkersWithRotMatrix */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("ros2_aruco_interfaces.msg._aruco_markers_with_rot_matrix");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "ArucoMarkersWithRotMatrix");
  ...
  {  // rot_mat
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "rot_mat");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    if (ros_message->rot_mat.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      double * src = &(ros_message->rot_mat.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->rot_mat.size * sizeof(double));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
